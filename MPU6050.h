...
long acc_x, acc_y, acc_z, acc_total_vector, acc_X_Z_vector, acc_Y_Z_vector;
int temperature;
long gyro_x_cal, gyro_y_cal, gyro_z_cal, acc_total_vector_cal;
long loop_timer;
...
void MpuDebug() {
  ...
  if (millis() - lastPrint >= 250) {
    ...
    Serial.print("   acc x: ");
    Serial.print(acc_x);    //Print x acceleration
    Serial.print("   acc y: ");
    Serial.print(acc_y);    //Print y acceleration
    Serial.print("   acc z: ");
    Serial.println(acc_z);    //Print z acceleration
    
    Serial.print("   vector: ");
    Serial.print(acc_total_vector);    //Print vector (this is the earth acceleration)
    Serial.print("   vector xz: ");
    Serial.print(acc_X_Z_vector);    //Print vector
    Serial.print("   vector yz: ");
    Serial.println(acc_Y_Z_vector);    //Print vector

    Serial.print("   along: ");
    Serial.print(along);    //Print vector
    Serial.print("   across: ");
    Serial.println(across);    //Print vector
    Serial.print("   vertical: ");
    Serial.println(vertical);    //Print vector
  }
#endif
}
...
void processMpu6050Data() {
  ...
  acc_total_vector = sqrt((acc_x * acc_x) + (acc_y * acc_y) + (acc_z * acc_z)); // Calculate the total accelerometer vector (= earth acceleration)
  //57.296 = 1 / (3.142 / 180) The Arduino asin function is in radians

   // Calibrate vertical acceleration vector (zero offset)
  if (acc_total_vector_cal == 0) acc_total_vector_cal = acc_total_vector;

  // Calculate vertical acceleration
  vertical = acc_total_vector_cal - acc_total_vector; 

  // Calculate accereometer based pitch and roll angles
  ...
  // Calculate acceleration vectors for X/Z and Y/Z
  acc_X_Z_vector = sqrt((acc_x * acc_x) + (acc_z * acc_z));
  acc_Y_Z_vector = sqrt((acc_y * acc_y) + (acc_z * acc_z));

  // Calculate gyro compensated angles
  ...
  // Calculate angle compensated accelerations for servo movements
  along = acc_y - (sin(angle_pitch / 57.296) * acc_Y_Z_vector);
  across = acc_x - (sin(angle_roll / -57.296) * acc_X_Z_vector);
}
...
void setupMpu6050() {
  ...
  if (error > 0) {
#ifdef DEBUG// Read the I2C error byte
...
#ifdef DEBUG
  Serial.println("Do not move vehicle...");                            // Print text to console
#endif
